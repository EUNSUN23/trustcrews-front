...
import { useDeleteTask } from '@/features/core/task/api/deleteTask';
import {
  TaskModForm,
...
import { TASK_STATUS } from '@/entities/task/taskStatus';
...
const TaskCardMenu = ({ taskItem, userPMAuthCode }: TaskCardMenuProps) => {
  ...
  const handleClickDeleteButton = (e: MouseEvent) => {
    ...
    if (confirm('업무를 삭제하시겠습니까?')) {
      deleteTask({ taskId, userPMAuth: userPMAuthCode });
    }
  };
